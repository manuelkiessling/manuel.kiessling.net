...
Die Abfrage von Ereignisdaten erfolgt dabei fachlich oft strukturiert und limitiert nach Zeit, beispielsweise im Sinne von "Berechne die Anzahl aller Neuregistrierungen der vergangen 7 Tage".
...
Obwohl man zum Beispiel lediglich die Titel aller Kriminalromane aus dem Jahr 1998 ermitteln möchte, wäre man dennoch gezwungen, jedes einzelne Buch einmal in die Hand zu nehmen und auf die Eigenschaften "ist von 1998" und "ist ein Kriminalroman" zu untersuchen.

# Der Lösungsansatz

Wären die Bücher im Raum aber physischen nach einem logischen Kriterium partitioniert — zum Beispiel: alle Bücher eines Jahres stehen immer in einem eigenen Regal für dieses Erscheinungsjahr — dann wäre die Ermittlung der gewünschten Titel viel effizienter, da man jetzt nur noch alle Bücher vom Regal "1998" auf die Eigenschaft "ist ein Kriminalroman" untersuchen muss.
...
hat dann nicht nur auf SQL-Ebene eine bestimmte Filterung des Resultsets zur Folge, sondern wirkt sich auch auf das Zugriffsmuster auf die physischen Dateien in S3 aus: Dank der Partitionierung versteht Athena, dass nur die beiden Dateien 

    datawarehouse/business-events/year=2024/month=09/day=02/file4.parquet
    datawarehouse/business-events/year=2024/month=09/day=02/file5.parquet

physisch gelesen werden müssen, um die Anfrage fachlich korrekt und vollständig zu beantworten. Alle anderen Dateien können gefahrlos ignoriert werden.
...
Der Datensatz eines Ereignisses, das in einer Anwendungsplattform am 1. September aufgetreten ist, kann in diesem Fall also in der S3 Ordnerstruktur vom 2. September abgelegt werden, wenn aus irgendwelchen Gründen eine längere Verzögerung zwischen Eintritt des Ereignisses und dessen Data Warehouse Verarbeitung liegt.
...
- Der Name der Tabelle lautet "business-events-as-json", sie gehört zur Glue Database "business-events".
- Das Table Format ist "Standard AWS Glue table".
- Das Data Format ist "JSON"
- Für das Glue Schema müssen nun alle Felder des angelieferten JSON, inklusive der drei Datumsfelder `year`, `month`, und `day`, angegeben werden. Dies geschieht am einfachsten per JSON-Definition:
...
- Als "Schema for source records" definieren wir nun die zuvor angelegte Glue Table, durch Angabe der korrekten Glue Region, Database, und Table business-events-as-json.
...
- Diese Key Names kommen nun im Feld "S3 Bucket Prefix" wieder zum Tragen, denn dort geben wir an:
  `business-events/year=!{partitionKeyFromQuery:year}/month=!{partitionKeyFromQuery:month}/day=!{partitionKeyFromQuery:day}/`
...
Nun können Events an diesen Stream eingeliefert werden, diese müssen dann nach entsprechender Verzögerung (dem Buffer Interval) in den korrekten Unterordnern in S3 auftauchen.
